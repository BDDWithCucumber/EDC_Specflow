Feature: ODM_Labs Yes to No and Yes to No
...
Scenario: T14_Verify the XML Data evented out for the Item Data is entered via Flag Update
	Given I save the Event XML data in "StudyPublishActual01.xml" file by executing the query "select TOP 1 XmlMessage from OdmSubscriberDetail where StudyEventKey = 8 order by OdmSubscriberDetailKey desc"
	And I have the XML file "StudyPublishExpected01.xml" available in the project folder.
	And I have the XML file "StudyPublishActual01.xml" available in the Test XML Files folder.
...
Scenario: T18_Verify the XML Data evented out for the Item Data is entered via Flag Update
	Given I save the Event XML data in "StudyPublishActual02.xml" file by executing the query "select TOP 1 XmlMessage from OdmSubscriberDetail where StudyEventKey = 8 order by OdmSubscriberDetailKey desc"
	And I have the XML file "StudyPublishExpected02.xml" available in the project folder.
	And I have the XML file "StudyPublishActual02.xml" available in the Test XML Files folder.
	Then I verify that the data in both XML files is same except for below tags and fields.
	| TagName | FieldName           |
	|         | CreationDateTime    |
	|         | DateTimeStamp       |
...
Scenario: T22_Verify the XML Data evented out for the Item Data is entered via Flag Update
	Given I save the Event XML data in "StudyPublishActual03.xml" file by executing the query "select TOP 1 XmlMessage from OdmSubscriberDetail where StudyEventKey = 8 order by OdmSubscriberDetailKey desc"
	And I have the XML file "StudyPublishExpected03.xml" available in the project folder.
	And I have the XML file "StudyPublishActual03.xml" available in the Test XML Files folder.
	Then I verify that the data in both XML files is same except for below tags and fields.
	| TagName | FieldName           |
	|         | CreationDateTime    |
	|         | DateTimeStamp       |
...
Scenario: T25_Verify the XML Data evented out for the Item Data is entered via Flag Update
	Given I save the Event XML data in "StudyPublishActual04.xml" file by executing the query "select TOP 1 XmlMessage from OdmSubscriberDetail where StudyEventKey = 8 order by OdmSubscriberDetailKey desc"
	And I have the XML file "StudyPublishExpected04.xml" available in the project folder.
	And I have the XML file "StudyPublishActual04.xml" available in the Test XML Files folder.
	Then I verify that the data in both XML files is same except for below tags and fields.
	| TagName | FieldName           |
	|         | CreationDateTime    |
	|         | DateTimeStamp       |
...
@ObjectiveEvidence @ReqID:US67434.1
Scenario: T30_Verify the XML Data evented out for the Item Data is entered via Flag Update
	Given I save the Event XML data in "StudyPublishActual05.xml" file by executing the query "select TOP 1 XmlMessage from OdmSubscriberDetail where StudyEventKey = 8 order by OdmSubscriberDetailKey desc"
	And I have the XML file "StudyPublishExpected05.xml" available in the project folder.
	And I have the XML file "StudyPublishActual05.xml" available in the Test XML Files folder.
	Then I verify that the data in both XML files is same except for below tags and fields.
	| TagName | FieldName           |
	|         | CreationDateTime    |
	|         | DateTimeStamp       |
...
@ObjectiveEvidence @ReqID:US67434.1
Scenario: T32_Verify the XML Data evented out for the Item Data is entered via Flag Update
	Given I save the Event XML data in "StudyPublishActual06.xml" file by executing the query "select TOP 1 XmlMessage from OdmSubscriberDetail where StudyEventKey = 8 order by OdmSubscriberDetailKey desc"
	And I have the XML file "StudyPublishExpected06.xml" available in the project folder.
	And I have the XML file "StudyPublishActual06.xml" available in the Test XML Files folder.
	Then I verify that the data in both XML files is same except for below tags and fields.
	| TagName | FieldName           |
	|         | CreationDateTime    |
	|         | DateTimeStamp       |
...
Scenario: T33_Expand the Visit (week2) event and open the FormD1 form and update the data.
	Given I navigate to "Patients" tab from main menu.
    And I select the site "01-Site" from Sites dropdown.
    And I select the patient "02-PAT" from patients table.
	And I expand "Visit (week2)" to select and open "FormD1" CRF.
...
@ObjectiveEvidence @ReqID:US67434.1
Scenario: T35_Verify the XML Data evented out for the Item Data is entered via Flag Update
	Given I save the Event XML data in "StudyPublishActual07.xml" file by executing the query "select TOP 1 XmlMessage from OdmSubscriberDetail where StudyEventKey = 8 order by OdmSubscriberDetailKey desc"
	And I have the XML file "StudyPublishExpected07.xml" available in the project folder.
	And I have the XML file "StudyPublishActual07.xml" available in the Test XML Files folder.
	Then I verify that the data in both XML files is same except for below tags and fields.
	| TagName | FieldName           |
	|         | CreationDateTime    |
	|         | DateTimeStamp       |
...
#Initial Data Setup
Scenario: T36_Delete a study
	When I delete study
	Then I see study label "No Study Loaded" in header

Scenario: T37_Upload and publish the study.
    When I click breadcrumb "Study Management"
    And I upload a study "StudyPublish.zip" with study label "AllmappedforD1_1"
    Then I see study label "AllmappedforD1_1" in header 

Scenario: T38_Add a new lab test reference range record under Lab02 under WBC
	Given I navigate to "Local Labs" tab from main menu.
    When I open lab with labid "L02" from ManageReferenceRanges page
	And I open labtest with labname "MRRTEST1" from labtests page
	And I click add and enter reference ranges of a lab test and Save and can see "LabTest Reference Range Added Successfully" message upon each record save in ReferenceRangesPage including NA
...
Scenario: T39_Add patient 01-PAT from Datalabs UI
      Given I navigate to "Patients" tab from main menu.
      And I select the site "01-Site" from Sites dropdown.
      When I add patient with below details.
      | ScreenID | PatientInitials | ScreenDate |
...
Scenario: T40_Select Site1 and patient 01-PAT.
	Given I navigate to "Patients" tab from main menu.
	And I select the site "01-Site" from Sites dropdown.
	When I select the patient "01-PAT" from patients table.
	Then I see Patient details page for "01-PAT".

Scenario: T41_Expand the Visit (Screening) and open the demographics form to update the data
	When I expand "Event 1" to select and open "Demographics" CRF.
    And I click "Checkout" button on "Demographics" CRF.      
    And I enter below flat questions data in the CRF.
    | QuestionPrompt      | AnswerValue | AnswerFieldType |
...
Scenario: T42_Expand the Visit (week2) event and open the FormD1 form and update the data.
	When I expand "Visit (week2)" to select and open "FormD1" CRF.
	And I click "Checkout" button on "FormD1" CRF. 
	And I enter below flat questions data in the CRF.
	| QuestionPrompt                | AnswerValue | AnswerFieldType |
...
@ObjectiveEvidence @ReqID:US67434.1
Scenario: T43_Verify the XML Data evented out for the Item Data is entered via Flag Update
	Given I save the Event XML data in "StudyPublishActual08.xml" file by executing the query "select TOP 1 XmlMessage from OdmSubscriberDetail where StudyEventKey = 8 order by OdmSubscriberDetailKey desc"
	And I have the XML file "StudyPublishExpected08.xml" available in the project folder.
	And I have the XML file "StudyPublishActual08.xml" available in the Test XML Files folder.
	Then I verify that the data in both XML files is same except for below tags and fields.
	| TagName | FieldName           |
	|         | CreationDateTime    |
	|         | DateTimeStamp       |
...
Scenario: T44_Upload and Stage a Study which has major study changes
	When I select DataManagerPortal item "Study Administration" from drop down
	And I click Link "Study Management" In StudyAdministrationPage
	And I click ActionPalette Item "Manage Versions" in "StudyResetAction" ActionPalette
	And I click ActionPalette Item "Upload Study File" in "StudyVersionMgmtListActions" ActionPalette
...
Scenario: T45_Publish the study file that has Major changes	
	When I click ActionPalette Item "Publish" in "StudyVersionPropertiesActions" ActionPalette
	And I clicked Continue button in PublishStudyFlagsPage
...
@ObjectiveEvidence @ReqID:US67434.1
Scenario: T46_Verify the XML Data evented out for the Item Data is entered via Flag Update
...
Scenario: T47_Expand the Visit (week2) event and open the FormD1 form and update the data.
	Given I navigate to "Patients" tab from main menu.
    And I select the site "01-Site" from Sites dropdown.
    And I select the patient "01-PAT" from patients table.
	And I expand "Visit (week2)" to select and open "FormD1" CRF.
...
Scenario: T48_Upload and Stage a Study which has major study changes
	When I select DataManagerPortal item "Study Administration" from drop down
	And I click Link "Study Management" In StudyAdministrationPage
	And I click ActionPalette Item "Manage Versions" in "StudyResetAction" ActionPalette
	And I click ActionPalette Item "Upload Study File" in "StudyVersionMgmtListActions" ActionPalette
...
Scenario: T49_Publish the study file that has Major changes	
	When I click ActionPalette Item "Publish" in "StudyVersionPropertiesActions" ActionPalette
	And I clicked Continue button in PublishStudyFlagsPage
	And I select patient migration option "Migrate all patients" in PublishStudyPage
	And I click Publish button in PublishStudyPage
...
@ObjectiveEvidence @ReqID:US67434.1
Scenario: T50_Verify the XML Data evented out for the Item Data is entered via Flag Update
	Given I save the Event XML data in "StudyPublishActual10.xml" file by executing the query "select TOP 1 XmlMessage from OdmSubscriberDetail where StudyEventKey = 8 order by OdmSubscriberDetailKey desc"
	And I have the XML file "StudyPublishExpected10.xml" available in the project folder.
	And I have the XML file "StudyPublishActual10.xml" available in the Test XML Files folder.
	Then I verify that the data in both XML files is same except for below tags and fields.
	| TagName | FieldName           |
	|         | CreationDateTime    |
	|         | DateTimeStamp       |
...
Scenario: T51_Expand the Visit (week2) event and open the FormD1 form and update the data.
	Given I navigate to "Patients" tab from main menu.
    And I select the site "01-Site" from Sites dropdown.
    And I select the patient "01-PAT" from patients table.
	And I expand "Visit (week2)" to select and open "FormD1" CRF.
...
Scenario: T52_Verify the XML Data evented out for the Item Data is entered via Flag Update
	Given I save the Event XML data in "StudyPublishActual11.xml" file by executing the query "select TOP 1 XmlMessage from OdmSubscriberDetail where StudyEventKey = 8 order by OdmSubscriberDetailKey desc"
	And I have the XML file "StudyPublishExpected11.xml" available in the project folder.
	And I have the XML file "StudyPublishActual11.xml" available in the Test XML Files folder.
	Then I verify that the data in both XML files is same except for below tags and fields.
	| TagName | FieldName           |
	|         | CreationDateTime    |
	|         | DateTimeStamp       |
