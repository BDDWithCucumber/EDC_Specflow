Feature: DS: Create Form from existing: Include and use existing for domain, domain items on save
  Acceptance Criteria:
...
On clicking save, the system shall generate a new form with the duplicate domain and domain items (when source and target fields are same and there are no errors with the Form name, Form Label and Table name).
...
Scenario: T02_When I Import Study of 5.7 to Designer then I see message "Study has been successfully added" and status as complete
	...
	And I import a study "TS4500_Base1.zip" 
