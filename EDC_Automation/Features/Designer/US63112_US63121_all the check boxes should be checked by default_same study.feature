...
Scenario: T02_When I Import Study of 5.9 to Designer then I see message "Study has been successfully added" and status as complete
	...
	Then I should see the status of the study "Study Import (Import_Study59)" in "Complete" state
...
Scenario: T04_When user opens the pop up page then all the checkboxes should be checked by default and the save button should be in disabled state
	...
	And I click on the Link "Import_Study59"
...
@ObjectiveEvidence @ReqID:3.1.2.3
Scenario: T05_When I click on Verify for the same study then the System should highlight all the duplicate fields in orange color and unchecked and the Include checkbox should be disabled
...
Scenario: T06_When the user renames the Domain Item then the Domain Item should be automatically checked by the system upon rename 
...
Scenario: T07_When I click on Create Form from Existing and select a same study then verify button should be displayed
...
Scenario: T08_When user renames the first codelist item then all the similar codelist items will be renamed
	When I click on Verify button
	Then I should see the renamed Codelist as below
	| Row | TargetForCodelist |
	| 11  | XYN_Rename1        |
...
Scenario: T09_When user unchecks the Domain Item then validation should not happen for the entire record upon verify
...
Scenario: T10_When the user checks  
...
Scenario: T11_When the user renames the Domain Item name then the system should throw the error that the Item Name is already in use
	When I click on Verify button
	And I rename the DomainItem as below
	| Row | SourceForDoamin | TargetForDomain | TargetRenameForDomain  |
...
Scenario: T12_When the user renames the Domain name then the system should throw the already in use error upon verify
	When I click on Verify button
...
Scenario: T14_When the user unchecks the Include checkbox of Codelist then validation should not happen for Codelist upon verify
	When I click on the Link "Create Form from Existing"	
	And I select the pop up having title "Create Form from Existing" and partial message "Please select a Study"
	And I click on the Link "Import_Study59"
	And I click on the Link "AutoBuild_Form03"	
...
Scenario: 15_When I navigate back to the Home page and click on Delete all previously created studies then I should see all those studies	
